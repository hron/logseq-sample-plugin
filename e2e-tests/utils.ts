...
export async function loadLocalGraph(page: Page, path: string): Promise<void> {
  ...
  await expect(
    page.locator('.notification-close-button').first()
 ...
 */
export async function captureConsoleWithPrefix(
  page: Page,
  prefix: string,
  timeout: number = 3000
): Promise<string> {
...
export async function queryPermission(
  ...
  return await page.evaluate(
    async (eval_permission: PermissionName): Promise<boolean> => {
 ...
 */
export async function repeatKeyPress(
  page: Page,
  key: string,
  times: number
): Promise<void> {
